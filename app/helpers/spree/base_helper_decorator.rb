  ...
  BaseHelper.class_eval do
    ...
    def expand_properties_facet_to_facet_array(facet)
      ...
      facet.body['terms'].each do |term|
        t = term['term'].split('||')
        property_name = t[0]
        property_value = t[1] ||= 'keine Angabe'
        next unless property_value.present?
      ...
      result = []
      block_properties = ['Stück pro Paket', 'Preisklasse', 'Kollektion', 'Inhalt m² pro Paket']
      property_names.reject{|k, v| block_properties.include?(k)}.each do |key,value| #
